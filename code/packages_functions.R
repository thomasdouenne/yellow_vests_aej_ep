...
save_plotly <- function(plot, filename = deparse(substitute(plot)), folder = '../images/', width = dev.size('px')[1], height = dev.size('px')[2], method='orca', trim = T, transparent = F) {
  ...
  if (transparent) image_write(image_transparent(image_read(file), 'white'), file)
}
